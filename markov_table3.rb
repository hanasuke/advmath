...
class MarkovTable3
  ...
  def generate(tweets_csv_file)
    ...
    CSV.foreach(tweets_csv_file) do |tweet|
      ...
      tmp.each do |t|
        w3 = t
        create_markov_table(w1, w2, w3)
        w1, w2  = w2, w3
      end
    end
  end

  def create_markov_table(w1, w2, w3)
    return if w3 == BEGIN_WORD
    if @word_list[w1][w2].nil?
      @word_list[w1][w2] = Array.new([w3])
    else
      @word_list[w1][w2] << w3
    end
  end
